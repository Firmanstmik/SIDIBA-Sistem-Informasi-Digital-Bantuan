...
class MonevReportService
{
    /**
     * Generate individual report HTML
     */
    public function generateIndividualReport(Beneficiary $beneficiary)
    {
        $totalMonev = $beneficiary->monevs->count();
        $lastMonev = $beneficiary->monevs->first();
        
        $html = $this->getReportHeader($beneficiary);
        $html .= $this->getBeneficiaryInfoSection($beneficiary);
        $html .= $this->getSummarySection($beneficiary, $totalMonev, $lastMonev);
        $html .= $this->getMonevHistorySection($beneficiary);
        $html .= $this->getRecommendationStatsSection($beneficiary, $totalMonev);
     ...
     * Get report header
     */
    private function getReportHeader($beneficiary)
    {
        return '<!DOCTYPE html>
        <html lang="id">
        <head>
            ...
            <title>Laporan Monev - ' . htmlspecialchars($beneficiary->nama) . '</title>
            <style>
    ...
    /**
     * Get beneficiary information section
     */
    private function getBeneficiaryInfoSection($beneficiary)
    {
        return '
            <div class="info-section">
                ...
                <div class="info-grid">
                    <div class="info-item">
                        <span class="info-label">Nama:</span> ' . htmlspecialchars($beneficiary->nama) . '
                    </div>
                    <div class="info-item">
                        <span class="info-label">NIK:</span> ' . htmlspecialchars($beneficiary->nik) . '
                    </div>
                    <div class="info-item">
                        <span class="info-label">Nomor HP:</span> ' . ($beneficiary->nomor_hape ?: '-') . '
                    </div>
                    <div class="info-item">
                        <span class="info-label">Bidang:</span> ' . htmlspecialchars($beneficiary->bidang) . '
                    </div>
                    <div class="info-item">
                        <span class="info-label">Jenis Bantuan:</span> ' . htmlspecialchars($beneficiary->jenis_bantuan) . '
                    </div>
                    <div class="info-item">
                    ...
                    <div class="info-item">
                        <span class="info-label">Alamat:</span> ' . htmlspecialchars($beneficiary->alamat) . '
                    </div>
                </div>
            </div>';
    }
     ...
     * Get summary section
     */
    private function getSummarySection($beneficiary, $totalMonev, $lastMonev)
    {
        return '
            <div class="summary-box">
                ...
                <div class="summary-grid">
                    <div class="summary-item">
                        <div class="summary-number">' . $totalMonev . '</div>
                        <div class="summary-label">Total Monev</div>
                    </div>
                    <div class="summary-item">
                        <div class="summary-number">' . ($lastMonev ? \Carbon\Carbon::parse($lastMonev->tanggal_monev)->format('d/m/Y') : '-') . '</div>
    ...
    /**
     * Get monev history section
     */
    private function getMonevHistorySection($beneficiary)
    {
        ...
        foreach ($beneficiary->monevs as $index => $monev) {
            ...
            $html .= '
                <tr>
                    ...
                    <td>' . htmlspecialchars($monev->pelaksana) . '</td>
                    <td>' . htmlspecialchars(Str::limit($monev->hasil_evaluasi, 100)) . '</td>
    ...
    /**
     * Get recommendation statistics section
     */
    private function getRecommendationStatsSection($beneficiary, $totalMonev)
    {
